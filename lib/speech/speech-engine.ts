...
/**
 * Browser speech engine for the boardroom.
 *
 ...
 * rejects. Callers use it to pace the debate, and a rejected promise there
 ...
 * caller needs to await this before assigning voices or the whole board ends
...
function loadVoices(): Promise<SpeechSynthesisVoice[]> {
  ...
  return new Promise((resolve) => {
    ...
    // (headless Chrome, a few Linux setups). Don't hang the debate on it.
...
async function voiceFor(speakerId: string): Promise<SpeechSynthesisVoice | null> {
  ...
  const profile = voiceProfiles[speakerId] ?? DEFAULT_VOICE_PROFILE;
 ...
 * currency and arrow glyphs the executives use get read as their Unicode
...
export interface SpeakOptions {
  /** Roster id, used to keep one voice per executive across the session. */
  speakerId: string;
...
export async function speak(text: string, options: SpeakOptions): Promise<void> {
  ...
  const profile = options.profile ?? voiceProfiles[options.speakerId] ?? DEFAULT_VOICE_PROFILE;
  ...
  try {
    for (const piece of chunks) {
      ...
      await new Promise<void>((resolve) => {
        ...
        // A failed utterance must not stall the debate — resolve and move on.
 ...
 * executive turn isn't swallowed.
 */
