 ...
 * rather than a dependency. Everything downstream — `app/api/speech/route.ts`
 * and the client hook — is built so that a failure here degrades silently to
 * the browser's own Web Speech API rather than leaving the board mute.
 *
 ...
 * produces the same voice on every machine, so the board sounds identical
...
import { edgeVoiceFor } from "@/lib/speech/edge-voices";
 ...
 * in front of playback means one slow synthesis stalls the meeting. Past
...
function buildSsml(text: string, executiveId: string): string {
  const voice = edgeVoiceFor(executiveId);
  return (
...
/**
 * Synthesises `text` in `executiveId`'s voice and resolves to MP3 bytes.
 *
 ...
 */
export async function synthesise(text: string, executiveId: string): Promise<Buffer> {
