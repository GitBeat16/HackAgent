 ...
 * every Next runtime.
 *
 * Nothing outside `lib/ai/` should import this file. Personas go through
 * `lib/ai/client.ts`; structured deliverables go through
 * `lib/ai/report-generator.ts` and `lib/ai/deliverables.ts`.
 */
...
export class GroqError extends Error {
  ...
  /** True when the board hit a per-minute token or request ceiling. */
  get isRateLimit() {
 ...
 *
 * A board session issues one request per executive turn with nothing
  ...
  const schemaAttempt = await post(
    {
      ...
      response_format: {
        type: "json_schema",
        json_schema: { name: "board_output", schema: strict, strict: true },
      },
    },
    options.signal,
  );
