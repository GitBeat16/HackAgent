...
import { isSpeechSupported, primeSpeech } from "@/lib/speech/speech-engine";
import { edgeAvailable, speak, stop, type SpeechEngine } from "@/lib/speech/speech-provider";

/** Speech support never changes for the life of the document. */
const subscribeToNothing = () => () => {};

/**
 * Reads the board's turns aloud.
 *
 * Prefers Edge's neural voices via `/api/speech` and drops to the browser's
 * own synthesiser if that fails — see `speech-provider.ts` for why the
 ...
 */
export function useBoardSpeech() {
  ...
  const browserSupported = useSyncExternalStore(subscribeToNothing, isSpeechSupported, () => false);
  ...
  // debate loop that depends on it.
  const mutedRef = useRef(muted);
  useEffect(() => {
    mutedRef.current = muted;
  }, [muted]);

  /** Aborts the in-flight fetch and playback when the founder interrupts. */
  ...
  // Nothing should keep talking after the founder navigates away.
   ...
   *
   * Resolves immediately when muted, so the debate loop can await it
   * unconditionally.
   */
  const speakMessage = useCallback(
    async (text: string, speakerId: string, isFounder = false) => {
      ...
      try {
        const used = await speak(
          { text, speakerId, isFounder, onStart: () => setVoicingId(speakerId) },
          controller.signal,
        );
  ...
  return { supported, muted, toggleMuted, voicingId, speakMessage, stop: stopSpeech, engine };
}
